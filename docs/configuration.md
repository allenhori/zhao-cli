...
contains a path separator (or starts with `~`); `dbt`, `uv run dbt` and absolute paths behave
exactly as before. A relative path given on the CLI via `--dbt-command` is resolved like any
shell argument, against the directory you run `zhao` from.
