...
#[test]
fn project_local_relative_dbt_command_runs_from_an_unrelated_working_directory() {
    let project = tempfile::tempdir().unwrap();
    write_dbt_project_marker(project.path());
    write_stub(&project.path().join("bin/mydbt"));
    std::fs::write(project.path().join("zhao.yml"), "dbt-command: bin/mydbt\n").unwrap();
    let elsewhere = tempfile::tempdir().unwrap();

    let output = run_lineage_compile(project.path(), elsewhere.path());

    assert_eq!(output.status.code(), Some(0), "{output:?}");
    assert!(project.path().join("stub-ran").exists());
}
...
fn root_level_relative_dbt_command_is_inherited_by_a_nested_project() {
    ...
    std::fs::write(
        repo.path().join("zhao.yml"),
        "dbt-command: ./.venv/bin/dbt\n",
    )
    .unwrap();
    let project = repo.path().join("analytics/project-a");
    std::fs::create_dir_all(&project).unwrap();
    write_dbt_project_marker(&project);
    // A project-local file that leaves dbt-command unset must still
    // inherit the root's, anchored at the repo root.
...
#[test]
fn a_relative_dbt_command_pointing_nowhere_fails_with_the_resolved_path() {
    let project = tempfile::tempdir().unwrap();
    write_dbt_project_marker(project.path());
    std::fs::write(
